...
const caseSetup: CaseSetup = {
  ...
  questionNodes: Array.from({ length: 10 }, (_, index) => ({
    id: `question-task-${index + 1}`,
    paperId: 1,
    title: `Investigation Task ${index + 1}`,
    prompt: `Question ${index + 1}?`,
    type: index === 0 ? 'claim' : index < 4 ? 'evidence' : index < 7 ? 'method' : 'limitation',
    status: 'open',
    parentQuestionId: null,
    dependsOnQuestionIds: index === 0 ? [] : [`question-task-${index}`],
    assignedEvidenceIds: [],
    position: {
      x: 120 + (index % 3) * 240,
  ...
  questionRelations: Array.from({ length: 9 }, (_, index) => ({
    id: `question-relation-task-${index + 1}-task-${index + 2}`,
    paperId: 1,
    sourceQuestionId: `question-task-${index + 1}`,
    targetQuestionId: `question-task-${index + 2}`,
    relationType: index < 3 ? 'support' : index < 6 ? 'method-for' : 'limitation-of',
  ...
  doctorState: {
    ...
    mode: 'skeptical',
    message: 'The first claim still needs direct evidence.',
...
describe('case investigation happy path', () => {
  beforeEach(() => {
    ...
    usePaperStore.setState({
      ...
      evidenceSubmissions: [],
      questionNodes: caseSetup.questionNodes,
      questionRelations: caseSetup.questionRelations,
      doctorState: caseSetup.doctorState,
      activeTaskId: 'task-1',
